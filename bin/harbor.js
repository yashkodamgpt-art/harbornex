...
const { setApiKey, getApiKey, loadConfig, CONFIG_FILE } = require('../src/config');
...
program
  .name('harbor')
  .description('HarborFlow - Turn your computer into a cloud')
  .version('0.1.0');

// Login
program.command('login')
  .description('Configure your Harbor API key')
  .argument('<api-key>', 'Your API key from harbor.dev/dashboard')
  .action((apiKey) => {
    setApiKey(apiKey);
    console.log(chalk.green('✓ API key saved!'));
    console.log(chalk.cyan('\nNext: Run `harbor register` to connect this device.'));
  });
