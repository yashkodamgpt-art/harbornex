...
export async function resedit(exePath, options) {
    ...
    if (options.iconPath) {
        ...
        Resource.IconGroupEntry.replaceIconsForResource(res.entries, existingIconGroups[0].id, existingIconGroups[0].lang, iconFile.icons.map((item) => item.data));
    }
    // Manifest
    if (options.win32Metadata?.['application-manifest'] ||
        ...
        if (options.win32Metadata?.['application-manifest']) {
            manifestEntry.bin = Buffer.from(await promisifiedGracefulFs.readFile(options.win32Metadata?.['application-manifest'])).buffer;
        }
