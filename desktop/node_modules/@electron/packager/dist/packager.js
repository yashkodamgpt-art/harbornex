...
async function createProcessedOptions(opts, platforms, packageJSONDir) {
...
function isValidProcessedOptions(opts) {
...
export class Packager {
    ...
    async testSymlink(comboOpts, zipPath) {
        ...
        const testPath = await fs.promises.mkdtemp(path.join(this.tempBase, `symlink-test-${comboOpts.platform}-${comboOpts.arch}-`));
    ...
    async getElectronZipPath(downloadOpts) {
        if (this.opts.electronZipDir) {
            if (fs.existsSync(this.opts.electronZipDir)) {
                const zipPath = path.resolve(this.opts.electronZipDir, `electron-v${downloadOpts.version}-${downloadOpts.platform}-${downloadOpts.arch}.zip`);
...
async function packageAllSpecifiedCombos(opts, archs, platforms) {
    ...
    return Promise.all(createDownloadCombos(opts, platforms, archs).map((downloadOpts) => packager.packageForPlatformAndArch(downloadOpts)));
}
/**
...
export async function packager(opts) {
    ...
    await runHooks(processedOpts.afterFinalizePackageTargets, createPlatformArchPairs(processedOpts, platforms, archs).map(([platform, arch]) => ({
        platform,
        arch,
    })));
