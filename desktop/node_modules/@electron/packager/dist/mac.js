...
export class MacApp extends App {
    ...
    async determinePlistFilesToUpdate() {
        ...
        const optional = await Promise.all(possiblePlists.map(async (item) => fs.existsSync(item[0]) ? item : null));
        return [
            ...plists,
    ...
    async updatePlistFiles() {
        ...
        await Promise.all(plists.map(([filename, varName]) => promisifiedGracefulFs.writeFile(filename, plist.build(this[varName]))));
    }
    async moveHelpers() {
        const helpers = [
            ' Helper',
    ...
    async moveHelper(helperDirectory, suffix) {
        ...
        if (fs.existsSync(path.join(helperDirectory, `${originalBasename}.app`))) {
            return this.renameHelperAndExecutable(helperDirectory, originalBasename, `${sanitizeAppName(this.appName)}${suffix}`);
        }
        else {
            return Promise.resolve();
        }
    ...
    async copyIconComposerIcon(appPlist) {
        ...
        if (iconComposerIcon) {
            ...
            await promisifiedGracefulFs.writeFile(path.join(this.originalResourcesDir, 'Assets.car'), assetCatalog);
        }
    }
    async copyIcon() {
        if (!this.opts.icon) {
        ...
        if (icon) {
            ...
            await fs.promises.cp(icon, path.join(this.originalResourcesDir, this.appPlist.CFBundleIconFile));
        }
    }
