...
export function createPlatformArchPairs(opts, selectedPlatforms, selectedArchs, ignoreFunc) {
    ...
    for (const arch of selectedArchs) {
        if (arch === 'universal' && process.platform !== 'darwin') {
            debug(`Skipping universal arch for ${process.platform} because @electron/universal only works on darwin`);
            continue;
        }
...
function validOfficialPlatformArch(platform, arch) {
    return (officialPlatformArchCombos[platform] &&
