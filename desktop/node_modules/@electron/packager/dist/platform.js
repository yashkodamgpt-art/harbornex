...
import { createPackageWithOptions as createASARWithOptions, getRawHeader, } from '@electron/asar';
import { baseTempDir, createAsarOpts, debug, ensureArray, generateFinalPath, validateElectronApp, warning, } from './common.js';
import { userPathFilter } from './copy-filter.js';
import { runHooks } from './hooks.js';
import crypto from 'node:crypto';
export class App {
    asarIntegrity = undefined;
    asarOptions;
    cachedStagingPath = undefined;
    opts;
    templatePath;
    ...
    async copyTemplate() {
        ...
        await fs.promises.cp(this.opts.dir, this.originalResourcesAppDir, {
            ...
            dereference: typeof this.opts.derefSymlinks === 'boolean'
    ...
    async removeDefaultApp() {
        await Promise.all(['default_app', 'default_app.asar'].map(async (basename) => fs.promises.rm(path.join(this.originalResourcesDir, basename), {
            recursive: true,
            force: true,
        })));
    }
    ...
    async normalizeIconExtension(targetExt) {
        ...
        /* istanbul ignore next */
        warning(`Could not find icon "${this.opts.icon}" with extension "${targetExt}", skipping this app icon format`, this.opts.quiet);
    }
    ...
    async copyExtraResources() {
        ...
        await Promise.all(extraResources.map((resource) => fs.promises.cp(resource, path.resolve(this.stagingPath, this.resourcesDir, path.basename(resource)))));
