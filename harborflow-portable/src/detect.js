...
// Load harbor.json or nexflow.json config if exists
function loadHarborConfig(projectPath) {
    // Check for harbor.json first, then nexflow.json
    const configNames = ['harbor.json', 'nexflow.json'];
    ...
    for (const configName of configNames) {
        ...
        if (fs.existsSync(configPath)) {
            try {
                ...
                console.log(`[NexFlow] Found ${configName}: ${config.name || 'unnamed'}`);
                return config;
            } catch (e) {
                console.error(`[NexFlow] Error reading ${configName}:`, e.message);
                return null;
            }
        }
    }
...
function detectProjectType(projectPath) {
    // First, check for harbor.json / nexflow.json
    const harborConfig = loadHarborConfig(projectPath);
    if (harborConfig) {
        // Support both formats: { startCommand: "..." } and { start: { command: "..." } }
        const startCmd = harborConfig.startCommand || harborConfig.start?.command || 'npm start';
        const buildCmd = harborConfig.buildCommand || harborConfig.build?.command || null;
        const port = harborConfig.port || harborConfig.start?.port || 3000;
